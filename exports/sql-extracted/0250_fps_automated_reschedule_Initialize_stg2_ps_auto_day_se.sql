...
-- Component: Initialize stg2.ps_auto_day_seven
-- Type: SQL Query
-- Job ID: 4650671
-- Component ID: 4650919
...
, day_7_ranking as (
    select
        ...
        ,(case when day1_r.sku_without_merv_rating is null then 1 else 0 end)
            ...
            + (case when day4_r.sku_without_merv_rating is null then 1 else 0 end)
            + (case when day5_r.sku_without_merv_rating is null then 1 else 0 end)
            + (case when day6_r.sku_without_merv_rating is null then 1 else 0 end) as prior_not_scheduled_skus
    ...
    from ranking r
        ...
        left join ${stg2_schema}.ps_auto_day_five day5 on day5.mapped_manufacturing_location = r.manufacturing_location
            and day5.sku_without_merv_rating = r.sku_without_merv_rating
            and day5.original_scheduled_dt = r.original_scheduled_dt
        left join ${stg2_schema}.ps_auto_day_six day6 on day6.mapped_manufacturing_location = r.manufacturing_location
            and day6.sku_without_merv_rating = r.sku_without_merv_rating
            and day6.original_scheduled_dt = r.original_scheduled_dt
        ...
        left join (select distinct mapped_manufacturing_location, sku_without_merv_rating from ${stg2_schema}.ps_auto_day_five) day5_r on day5_r.mapped_manufacturing_location = r.manufacturing_location
            and day5_r.sku_without_merv_rating = r.sku_without_merv_rating
        left join (select distinct mapped_manufacturing_location, sku_without_merv_rating from ${stg2_schema}.ps_auto_day_six) day6_r on day6_r.mapped_manufacturing_location = r.manufacturing_location
            and day6_r.sku_without_merv_rating = r.sku_without_merv_rating
    where day1.sku_without_merv_rating is null
        ...
        and day5.sku_without_merv_rating is null
        and day6.sku_without_merv_rating is null
)
...
from
    ...
    from base_staffing bs
        join day_7_ranking r on r.manufacturing_location = bs.mapped_manufacturing_location
            and r.rn_over_sku_without_merv_rating = bs.rn
    where bs.rn = 7 /* 7th date in period */) t
