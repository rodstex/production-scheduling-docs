...
-- Component: Initialize stg2.ps_auto_day_one
-- Type: SQL Query
-- Job ID: 4650671
-- Component ID: 4650736
...
, base_records as (
    select
        false                       as is_already_scheduled
        ,manufacturing_location     as manufacturing_location
        ,inserted_dt_utc            as original_scheduled_dt
        ,sku_without_merv_rating    as sku_without_merv_rating
 ...
 * Order by (1) is already scheduled descending
 *          (2) production lines descending
 *          (3) original scheduled date ascending (included to prevent ties)
 *          (4) SKU without MERV rating ascending (including to prevent ties)
 * This order ensures that (1) the tool evaluates the schedule that will be rescheduled against the already-scheduled production.
 * This order also ensures that partial production runs (if any) are scheduled later in the week.
 ...
 * Assign any already-scheduled production to the zeroth rank.
...
, ranking as (
    select
        ...
        ,row_number() over (partition by br.manufacturing_location, br.sku_without_merv_rating
            order by br.is_already_scheduled desc ,br.production_lines desc, br.original_scheduled_dt asc, br.sku_without_merv_rating asc)  as rn_over_sku_without_merv_rating
        ...
        ,case when br.is_already_scheduled then 0
            else br1.rank_over_mfg_location end as rank_over_mfg_location
    from base_records br
        join
            (select
                ...
                ,row_number() over (partition by manufacturing_location
                    order by sum(production_lines) desc, sku_without_merv_rating) as rank_over_mfg_location
            from base_records
...
/* Create a new schedule for the 1st day in the period.
 * Join the 1st day in the period with the 1st record for each SKU without MERV rating.
 * Order all SKU without MERV ratings by the total production lines required descending.
 * Based on this ranking, only return the SKU without MERV ratings that are within staffing capacity.
 */
select
    mapped_manufacturing_location
    ,sku_without_merv_rating
...
from
    ...
    from base_staffing bs
        join ranking r on r.manufacturing_location = bs.mapped_manufacturing_location
            and r.rn_over_sku_without_merv_rating = bs.rn
    where bs.rn = 1 /* 1st date in period */) t
