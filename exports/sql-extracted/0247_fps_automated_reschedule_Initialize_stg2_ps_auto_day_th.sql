...
-- Component: Initialize stg2.ps_auto_day_three
-- Type: SQL Query
-- Job ID: 4650671
-- Component ID: 4650851
...
, day_3_ranking as (
    select
        ...
        ,(case when day1_r.sku_without_merv_rating is null then 1 else 0 end)
            + (case when day2_r.sku_without_merv_rating is null then 1 else 0 end) as prior_not_scheduled_skus
...
from
    ...
    from base_staffing bs
        join day_3_ranking r on r.manufacturing_location = bs.mapped_manufacturing_location
            and r.rn_over_sku_without_merv_rating = bs.rn
    where bs.rn = 3 /* 3rd date in period */) t
