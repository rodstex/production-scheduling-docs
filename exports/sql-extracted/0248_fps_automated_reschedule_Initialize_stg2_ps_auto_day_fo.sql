...
-- Component: Initialize stg2.ps_auto_day_four
-- Type: SQL Query
-- Job ID: 4650671
-- Component ID: 4650884
...
, day_4_ranking as (
    select
        ...
        ,(case when day1_r.sku_without_merv_rating is null then 1 else 0 end)
            ...
            + (case when day3_r.sku_without_merv_rating is null then 1 else 0 end) as prior_not_scheduled_skus
...
from
    ...
    from base_staffing bs
        join day_4_ranking r on r.manufacturing_location = bs.mapped_manufacturing_location
            and r.rn_over_sku_without_merv_rating = bs.rn
    where bs.rn = 4 /* 4th date in period */) t
