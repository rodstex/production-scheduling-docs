-- Job: ps_non_automated_outside_mfg_line_capacity_trans
-- Path: ROOT/filterbuy_dw/dwh/jobs/production_schedule_v4/non_automated_production
-- Component: SQL 0
-- Type: SQL Query
-- Job ID: 4437512
-- Component ID: 4437519

/* Identify all the records that have a greater production need than the manufacturing location has non-automated lines.
...
with base_records as (
    select
        ...
        ,line_type
        ,running_sum_lines_over_mfg_line_type
        ,row_number() over (partition by mapped_manufacturing_location, line_type order by running_sum_lines_over_mfg_line_type asc) as rn
    from ${stg2_schema}.ps_non_automated_production_schedule_staging
    where not(is_copied_fps)
        and running_sum_lines_over_mfg_line_type > count_of_lines_over_mfg_line_type
)
 ...
 * For example:
 * Woodland has 3 Double Loader production lines
...
, factored as (
    select
        ...
        ,t.running_sum_lines_over_mfg_line_type
        ,case when t.running_sum_lines_over_mfg_line_type > t.count_of_lines_over_mfg_line_type then true else false end as is_outside_goal
        ,coalesce(t.count_of_lines_over_mfg_line_type::float - lag(t.running_sum_lines_over_mfg_line_type) over (partition by t.mapped_manufacturing_location, t.line_type order by t.running_sum_lines_over_mfg_line_type asc)
            ,t.count_of_lines_over_mfg_line_type::float) as prior_lines_remaining
        ,case when t.running_sum_lines_over_mfg_line_type = br.running_sum_lines_over_mfg_line_type then true else false end as is_record_to_factor
        ...
        ,t.production_need - production_need_within_capacity as production_need_outside_capacity
...
, transformed as (
    ...
    from factored f
        join ${stg2_schema}.ps_non_automated_production_schedule_staging t on t.mapped_manufacturing_location = f.mapped_manufacturing_location
            ...
            and t.line_type = f.line_type
            and t.sku = f.sku
            and t.running_sum_lines_over_mfg_line_type = f.running_sum_lines_over_mfg_line_type
    ...
    from ${stg2_schema}.ps_non_automated_production_schedule_staging t
        left join base_records br on br.rn = 1
            ...
            and br.line_type = t.line_type
            and br.running_sum_lines_over_mfg_line_type = t.running_sum_lines_over_mfg_line_type
    where not(t.is_copied_fps)
        and t.running_sum_lines_over_mfg_line_type > t.count_of_lines_over_mfg_line_type
        and br.rn is null
    group by 1,2,3,4,5,6,7,8
        ,t.runtime_dt_utc, t.production_capacity,t.changeover_production_lines_per_size
)
...
, final as (
    select
        ...
        ,is_within_capacity
        ,runtime_dt_utc
        ,'Manufacturing Location Line Type Limit' as outside_capacity_logic_type
...
--             ,line_type
--             ,running_sum_lines_over_mfg_line_type
--             ,row_number() over (partition by mapped_manufacturing_location, line_type order by running_sum_lines_over_mfg_line_type desc) as rn
--         from ${stg2_schema}.ps_non_automated_production_schedule_staging
--         where not(is_copied_fps)
--             and running_sum_lines_over_mfg_line_type <= count_of_lines_over_mfg_line_type) t0
--         join ${stg2_schema}.ps_non_automated_production_schedule_staging t1 on not(t1.is_copied_fps)
--             and t1.running_sum_lines_over_mfg_line_type <= t1.count_of_lines_over_mfg_line_type
--             and t1.mapped_manufacturing_location = t0.mapped_manufacturing_location
--             and t1.line_type = t0.line_type
--             and t1.running_sum_lines_over_mfg_line_type = t0.running_sum_lines_over_mfg_line_type
--     where t0.rn = 1
-- )
