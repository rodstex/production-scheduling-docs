...
-- Component: Initialize stg2.ps_auto_day_two
-- Type: SQL Query
-- Job ID: 4650671
-- Component ID: 4650813
...
/* Re-calculate the row number for each SKU without MERV rating.
 * Add N to the original row number (over SKU w/o MERV rating).
 * N equals the number of times the SKU w/o MERV rating was scheduled on prior days.
 */
, day_2_ranking as (
    select
        ...
        ,(case when day1_r.sku_without_merv_rating is null then 1 else 0 end) as prior_not_scheduled_skus
...
/* Create a new schedule for the 2nd day in the period.
 * Join the 2nd day in the period with the 2nd record for each SKU without MERV rating
 *      OR if the SKU without MERV rating was not scheduled on the 1st day, join on the 1st record for the SKU without MERV rating.
...
from
    ...
    from base_staffing bs
        join day_2_ranking r on r.manufacturing_location = bs.mapped_manufacturing_location
            and r.rn_over_sku_without_merv_rating = bs.rn
    where bs.rn = 2 /* 2nd date in period */) t
