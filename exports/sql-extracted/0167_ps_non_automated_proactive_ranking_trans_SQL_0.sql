...
, base_records as (
    ...
    from ${stg2_schema}.ps_fact_manufacturing_need_future fmn
        ...
        -- Truncate to filter types made on non-automated lines --
        join ${stg2_schema}.ps_target_days_of_inventory_history tdih on tdih.is_selected_dt
            and tdih.grouped_line_type = 'Non-Automated'
...
, production_need_by_day_sku as (
    ...
    from base_records br
        join ${stg2_schema}.ps_settings_history psh on psh.is_selected_dt
    group by 1,2,3,4, psh.non_automated_proactive_minimum_production_run
    having production_need >= psh.non_automated_proactive_minimum_production_run
    order by production_need asc
)

/* For each manufacturing location, rank SKU without MERV rating(s) by:
 *      (1) Production need (descending)
 */
, base_rankings as (
    select
        mapped_manufacturing_location
